...
`npm run qa:alive` is red on purpose, and its failure list is the worklist: 9 still viewports at 390 and 5 at 1440, all on `/ai-brain` and `/ai-gtm` except `/ @844px` and `/ @5400px`. Until 30 August it passed everything, because its rule was `mean >= 0.15 || peak >= 8` and `peak` reads the busiest 0.05% of pixels: one forty-pixel instrument ticking in a frozen screen of text satisfied it. Four of the seven phone viewports on the homepage were passing exactly that way while reading a whole-screen mean of 0.012 to 0.060. Do not lower the floor to make it green; the named viewports are the thing to fix.
