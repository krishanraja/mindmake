...
- Promoted the correction pass through PR #173, merge `0653dfd48605c7d2b76b5a49e2e41870cc2baa99`, production `dpl_4wh4EJwvJbNUdGVyKmcCjPyd5f7U`. Merged on the owner's explicit instruction before the candidate browser matrix finished; the run on main for that commit is the verification of record, not a completed pre-merge check. Public readback at 19:22 UTC: the four code-split routes each serve their own stylesheet in the head, the homepage carries `mm-homepage-active` from the first byte, the case-record lede renders and the sitemap answers 26 URLs. Rollback target is `dpl_82UgoyZatcpdZLWfeGvQnsMPvcwd` at `a0cc571`.
- Split the release workflow into two tiers on the owner's instruction, after he said the wait was not sustainable against the number of small changes queued behind it. The source gates still run on every commit; the 208-case browser matrix now runs on a pull request into main and on main itself. Superseded branch runs cancel, never on main. An unfiltered `push` trigger had been firing a duplicate of every run, including a second macOS job. A first attempt also narrowed the browser install to each leg's engines; CI failed both legs in under two minutes, because `qa:homepage-release` launches all three engines whatever the leg is, and it was reverted. Measured from the job logs, the browser install is about ten seconds on a GitHub runner, so the saving claimed for it was not real.
