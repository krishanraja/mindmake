...
const baselineHtml = await readFile(new URL("./index-s2-lock.html", import.meta.url), "utf8");
const candidateHtml = await readFile(new URL("./index-s2-motion.html", import.meta.url), "utf8");
...
const normalizedCandidate = candidateHtml
  .replace("<title>BRAIN-MOTION-S1 / Living Record</title>", "<title>BRAIN-ALIGNMENT-S2-LOCK / Living Record</title>")
  .replace('    <link rel="stylesheet" href="./styles-s2-motion.css" />\r\n', "")
  .replace('    <link rel="stylesheet" href="./styles-s2-motion.css" />\n', "")
  .replace('    <script src="./script-s2-motion.js" defer></script>\r\n', "")
  .replace('    <script src="./script-s2-motion.js" defer></script>\n', "")
  .replace(openingFilm.replaceAll("\n", "\r\n"), "")
  .replace(openingFilm, "");
assert.equal(normalizedCandidate, baselineHtml, "Motion candidate contains an undeclared HTML change");
...
observations.push("The approved S2 baseline and lock remain byte-identical; the motion candidate adds only its title, one film scene, one stylesheet and one controller.");

const server = await createServer({ root: repoRoot, logLevel: "error", server: { host: "127.0.0.1", port: 0 } });
await server.listen();
const address = server.httpServer.address();
const origin = `http://127.0.0.1:${address.port}`;
const conceptUrl = `${origin}/prototypes/website-redesign-recovery/brain-signature/index-s2-motion.html`;
...
await desktopShot.screenshot({ path: `${conceptDir}desktop-s2-motion-1440.png`, fullPage: true });
...
await mobileShot.screenshot({ path: `${conceptDir}mobile-s2-motion-390.png`, fullPage: true });
...
await review.goto(`${origin}/prototypes/website-redesign-recovery/brain-signature/review-s2-motion.html`, { waitUntil: "networkidle" });
await review.evaluate(() => document.fonts.ready);
await review.screenshot({ path: `${conceptDir}paired-s2-motion.png` });
await review.close();
...
const outputFiles = ["index-s2-motion.html", "styles-s2-motion.css", "script-s2-motion.js", "check-s2-motion.mjs", "review-s2-motion.html", "desktop-s2-motion-1440.png", "mobile-s2-motion-390.png", "paired-s2-motion.png", "media/evidence-connects-poster.png"];
const hashes = {};
...
console.log(JSON.stringify({
  ...
  baselineHashes,
  lockHashes,
  declaredDelta: ["candidate title", "one additive opening film", "one additive motion stylesheet", "one additive motion controller"],
