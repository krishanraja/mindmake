...
## 2026-08-11: The estate reconciled to the storefront

**Context.** Advisory was retired in July 2026 in anticipation of a full-time role. The Teardown and Handover storefront shipped in early August anyway. The role then fell through on an immigration technicality rather than on the work, so advisory became the primary cash engine again on 11 August. The storefront was right and everything behind it was still selling a wound-down business.

**What changed:**
- **Two engagements, three currencies.** `src/lib/offers.ts` became the single source of truth: twelve explicit price literals, no derivation between currencies, no FX. `CurrencyContext` plus a native-radio `CurrencySwitcher` on `/teardown`, `/handover` and `/capital`. `src/test/price-single-source.test.ts` fails the build if a price string appears anywhere else in `src/`, `public/`, `scripts/` or `index.html`.
- **Generated surfaces.** `public/llms.txt` became generated by `scripts/generate-llms.mjs`, and `scripts/prerender.mjs` now interpolates every price from `offers.ts` behind a second guardrail.
- **Ten orphaned routes redirected**, with real 301s added to `vercel.json` for the first time (every prior redirect was a client-side `<Navigate>` returning 200). The page components moved to `src/_archive/`, excluded from `tsconfig.app.json` and eslint. `/capital` stayed live, repointed at portfolio companies.
- **Three false claims removed:** a fabricated 4.9-from-50-reviews `aggregateRating`, 26 invented proof entries that Mindy was drawing on to write client proposals, and fabricated San Francisco geo coordinates. A "Global Offices" block on `/contact` claiming Brooklyn, London and Sydney went with them.
- **Mindy rewritten and redeployed.** New pricing card in three currencies, routing forked on Teardown versus Handover, the recommendation contract's field renamed `range` to `price`. Putting real questions through the deployed function then found three defects the green suite could not see: she deflected a direct price question, quoted the wrong Handover band, and never knew which currency the page was showing. All three fixed in the prompt layer and the hook. The generated proposal had the matching defect, hiding a published fee behind the book-call exit.
- **Consent-gated testimonials.** `publishable_testimonials`, a `security_invoker` view over rows with recorded permission, plus column-level grants that reduced what the anon key can read on the base table from all eighteen columns to seven.
- **Documentation reconciled** across the whole of `project-documentation/`, and `CLAUDE.md` rewritten against the actual code with a "deliberately absent" table.

Full reasoning, with review triggers, in `DECISIONS_LOG.md`.
