...
- A round of Krish's phone feedback, Chromium only. Homepage: the hero lede reads "with the high standards" and "your" in the headline is underlined, drawn in on load; the history opener ("We've been making new technology the enemy for thousands of years.") read "not impactful enough ... too much like the text below it", so its headline now leads the chapter's scale, the lede steps back to cream and "It's all in the mind." stands on its own mint italic line (copy unchanged); the four eras hold one layout, each frame reserving the tallest era's height, because the three-line calculator answer pushed the phone copy up; the "When built right" heading revealed with a blur on a promoted layer, which Android Chrome painted as a stale copy of the masthead, so it now fades and rises without blur; the footer's review-preview bob and fade are off and its tablet route rail stays on screen. Every page now carries the homepage's masthead and footer (the shell logo was about 40% wider, its footer carried a Media/Contact grid and a longer copyright); Contact left the footer. Every "Subscribe for free" link goes to makeyourmindup.ai. Headings squashed into one side of the column on /ai-gtm, /blog and the /new-age-leadership decision balance lost their narrow caps (Krish's example, "Your first commercial team can be mostly agents.", goes from four lines to two on a phone). The /case-studies cards stopped spilling out of their frames on phones shorter than about 850px (the visual row could shrink to zero), and card 07, which had borrowed card 06's 14 to 3 figure, shows its own: eleven of fourteen tools struck, one system live, "11 cut · 1 live". The lead capture flow keeps every focused field above the phone keyboard and its labels and choices meet WCAG AA contrast.
- The attendance line over the /case-studies logo rail, "People from these organisations have joined our sessions", became a count that runs up to 4000+ on first view and then "leaders who have adopted mind/make thinking, from businesses including" (Krish, from an Android screenshot of /case-studies). This returns the count `04_PROOF.md` had retired; Krish approved it, and the section 6 evidence trail is still to be compiled. Reduced motion, no script and crawlers read the settled "4000+".
- Media links to makeyourmindup.ai, the publication's own site, on Krish's instruction: every menu and footer Media link on every page, the homepage's generated menus and footer, and the `/signal` and `/builder-economy` redirects (`vercel.json` and the in-app routes). "Subscribe for free" still opens the one-field form at mindmakerlive.substack.com/subscribe, which is where makeyourmindup.ai's own subscribe buttons go; makeyourmindup.ai/subscribe returns 404.
- Two follow-ups on `/ai-brain`, from Krish. Chapter 04 said "It is written in plain words you can read and correct, and it goes where you go.", which read as a document against his "dont suggest the brain is just a text file"; on "Agreed with your recommendations" it now says "You read and correct it in plain words, without code, and it goes where you go." He also confirmed that the private brief document and its email keep "It is not advice." (STATE.md already records it). Then, from a phone screenshot: "Also this section builds a bit too early On scroll". The month read its progress from the instrument's top crossing 85% of the viewport, so on a phone Week 1 lit under the action bar and Day 30 and all thirty days were lit while the last step was still off screen. It now lights each step as its own top reaches 72% of the viewport, the days keep pace, and the build is read again when a transition moves the instrument after the last scroll event (a landed laptop chapter had stayed one step short). Ledger `AI-BRAIN-MONTH-PACE-001`; the other stepped instruments keep their S4 timing.
- Dates on the reading pages (`/blog`, each read, each tip and the read cards) are now spelt out by `writtenOn` in `src/lib/ideaFormat.ts` instead of the runtime's locale data. After #221 merged, main CI's WebKit job (run 36263499046) failed `qa:release-routes` on the four older Building reads: their related cards now include the 26 September read, and Safari wrote "26 Sep 2026" where the server wrote "26 Sept 2026", so hydration threw and the server heading was replaced. The same class of fault was live for every reader west of London: in Chromium set to Los Angeles, the unfixed build threw hydration errors on every blog page (25 on `/blog`) and showed each date a day early; the fixed build throws none and shows the right dates. `src/test/written-dates.test.ts` guards it, and the lock is regenerated for `Answer.tsx`, `Blog.tsx` and `BlogPost.tsx`.
