...
On the homepage the reach chapter is its organisation state alone, one still screen that does not pin (r38). The homepage pins are practice (three, above 900px; below it the scenes flow one after another) and the six AI Brain benefits; `/new-age-leadership` also pins reach through both its states. Their native sticky geometry and scroll-driven state progression live in `leadershipChapters.ts`, with direct controls that move the page to a state. They pin under the homepage's fixed masthead, advance through the complete approved sequence, release at the end and reverse naturally. Reduced motion keeps the pins and drops transitions and films. Do not replace them with long stacked story panels or intercept wheel/touch input into an inescapable trap.
