...
const output = 'C:/Users/krish/.scratch/mindmake-start-here-interaction-s3';
const candidatePath = '/prototypes/website-redesign-recovery/start-here-intelligence/index-interaction-s3.html';
const reviewPath = '/prototypes/website-redesign-recovery/start-here-intelligence/review-interaction-s3.html';
...
async function inspectReview(browserType, name, matrix, capture = false) {
  ...
  try {
    for (const [width, height] of matrix) {
      ...
      if (capture && width === 1600 && height === 1040) await page.screenshot({ path: `${output}/paired-review-interaction-s3.png`, fullPage: true });
...
const files = ['index-interaction-s3.html', 'styles-interaction-s3.css', 'script-interaction-s3.js', 'review-interaction-s3.html', 'check-interaction-s3.mjs'];
...
console.log(JSON.stringify({
  artifact: 'START-HERE-INTERACTION-S3 / INSTRUMENTED WORKING FOLIO',
  origin,
  frozenS2: Object.keys(frozenS2),
