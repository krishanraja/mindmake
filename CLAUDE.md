...
`npm run qa:alive` is red on purpose, and its failure list is the worklist: 4 still viewports at 390 and 1 at 1440, which are the two try-it forms and `/ @844px`. Do not lower a floor to make it green; the named viewports are the thing to fix. Re-measured 1 September 2026 and unchanged, with every failing viewport identified by driving the page: three of the four at 390 are `.mm-try`, as is the one at 1440. `/ai-brain @2700px` at 1440 sits on the cell floor and has flipped either side of it across runs, which is a borderline reading of a screen that is genuinely still rather than a reason to move the floor.
...
The current baselines, recorded in `project-documentation/06_CURRENT_STATE.md`: 356 tests, 0 lint errors and 2 warnings, 0 type errors. Do not add new lint problems. Point the typecheck at `tsconfig.app.json`: the root config carries `"files": []` and checks nothing.
