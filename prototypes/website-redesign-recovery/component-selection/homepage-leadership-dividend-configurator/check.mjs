...
const route = "/prototypes/website-redesign-recovery/component-selection/homepage-leadership-dividend-configurator/index.html";
const evidence = "C:/Users/krish/.scratch/mindmake-homepage-leadership-dividend-console";
const failures = [];
...
try {
  ...
  for (const [engineName, engine] of Object.entries(engines)) {
    ...
    for (const viewport of viewports) {
      ...
      fail(await page.locator("button:visible:not(.dividend-frame button)").evaluateAll((buttons) => buttons.some((button) => button.getBoundingClientRect().height < 43.5)), `${prefix}: console button under 44px`);
      ...
      fail(await page.locator(".dividend-panel:visible img").evaluateAll((images) => images.some((image) => !image.complete || image.naturalWidth < 1)), `${prefix}: visible imagery failed`);
      fail(!(await page.locator(".dividend-frame:visible .is-focused").count()), `${prefix}: affected preview region is not highlighted`);
      ...
      if (engineName === "chromium") await page.screenshot({ path: `${evidence}/${viewport.name}-first.png`, fullPage: false });

      if (engineName === "chromium" && viewport.name === "desktop-1440") {
        await page.locator("[data-decision-note]").fill("Mobile must remain a single bounded composition.");
        for (let step = 0; step < 20; step += 1) {
          fail((await page.locator("[data-progress-count]").textContent()) !== `${String(step + 1).padStart(2, "0")} / 20`, `sequence ${step + 1}: wrong progress`);
          const options = page.locator(".decision-option:visible");
          fail((await options.count()) < 2, `sequence ${step + 1}: no usable choices`);
        ...
        fail(!(await page.locator(".review-list mark").filter({ hasText: "Mobile must remain a single bounded composition." }).isVisible()), "full decision-note text missing from review");
        await page.locator("#dividendComment").fill("Overall: preserve the chapter's pace.");
        await page.getByRole("button", { name: /Submit choices and notes/ }).click();
        fail(!(await page.getByRole("heading", { name: "Selections submitted.", exact: true }).isVisible()), "submission state missing");
        fail((await page.locator("#dividendComment").inputValue()) !== "Overall: preserve the chapter's pace.", "overall note missing from submitted readback");
        ...
        fail((await page.locator("[data-decision-note]").inputValue()) !== "Mobile must remain a single bounded composition.", "decision note did not persist");
        await page.locator('[data-mode="benefits"]').click();
        const before = await page.locator("[data-benefit-title]:visible").textContent();
        await page.locator("[data-benefit-next]:visible").click();
        const after = await page.locator("[data-benefit-title]:visible").textContent();
        fail(before === after, "benefit carousel did not move");
        await page.getByRole("button", { name: "Mobile", exact: true }).click();
        fail((await page.locator(".preview-stage").getAttribute("data-device-view")) !== "mobile", "mobile preview toggle failed");
        await page.locator('[data-mode="return"]').click();
        fail((await page.locator('.dividend-frame:visible').getAttribute("data-mode-view")) !== "return", "returned-hour view did not activate");
        await page.screenshot({ path: `${evidence}/desktop-shell-mobile-return.png`, fullPage: false });
      }
      await page.close();
    }
    await browser.close();
  ...
  console.log(JSON.stringify({ origin, evidence, checks: "3 engines, 9 viewport sessions, 20 decisions, verbatim feedback readback, submission persistence, three chapter views", failures }, null, 2));
