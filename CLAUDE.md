...
`npm run qa:alive` is red on purpose, and its failure list is the worklist: 4 still viewports at 390 and 1 at 1440. Do not lower a floor to make it green; the named viewports are the thing to fix. Re-measured 2 September 2026 by driving each page: `/ai-brain @3376px`, `/ai-brain @4220px` and `/ai-gtm @2532px` at 390 and `/ai-brain @3600px` at 1440 are all `.mm-try`, and `/ @844px` is the fifth. The first two are the same form panel sampled twice, because reviving the dead margins made that page about 40px taller and the gate's sample points are fixed. Splitting the try-it panels was predicted to wake them and did not, and shortening the copy around them does not either. A form screen is finished the moment it is drawn, and `Arrive` cannot help: the gate reads a page at rest, and an arrival has by then arrived. What those screens want is a set-piece, which is a design decision rather than a fix.
