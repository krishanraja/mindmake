...
- r41 (`quality/route-lock/approved-production-r41.json`): Krish's phone review of `/ai-brain`, `/ai-gtm`, `/`, `/case-studies`, `/answers` and `/blog`, plus the same patterns found elsewhere on his instruction to include them. The AI Brain prototype markup loses the hero plaque and note, the evidence readout, the inspector standing line, the source labels, the replay button, status line and caveat, and the ready-state truth bar. The case-study rail's cards were nested scroll boxes with `overscroll-behavior: contain` and `touch-action: pan-x`: a vertical swipe on a card could not scroll the page, reproduced with CDP touch events before the fix and scrolling 385px after it. The partial horizontal-swipe glitch Krish saw on a device did not reproduce in Chromium emulation; the fix removes the gesture trap that is its most likely cause.
