...
Both doors ask for the same four things, in the same component, with the same rules: first name, last name, work email and the part of the business they work in. The company comes out of the email's domain, so nobody types it twice, and a personal address is refused on the page on both doors with a message that puts the limitation on us rather than on the visitor. Server-side that refusal is repeated only on `/ai-brain`; `/ai-gtm`'s `submit-mindmake-brief` has no matching check yet, see open item 10. What each page does with those four things is different, and deliberately so.
...
10. **`submit-mindmake-brief` has no server-side personal-email refusal.** `mindmake-personal-read/core.ts` throws `personal-email` when the domain is a free-email one; `submit-mindmake-brief/core.ts` has no equivalent check, so a hand-crafted request to the `/ai-gtm` company-read pipeline bypassing the page's own client-side check would be researched, verified and delivered against a personal address. `01_CANON.md`, `05_LEAD_DELIVERY_SPEC.md` and this file all previously claimed the refusal was symmetric across both doors; that line is now corrected to describe the actual asymmetry, found by this pass on 30 August 2026. This is a code fix, not a documentation one.
11. **The naming law is violated in shipped code.** Found by this pass on 30 August 2026, all still live: `supabase/functions/get-ai-news/index.ts` falls back to the literal string `"Mindmaker"` as a source label and in its LLM system prompt when a headline has no source (this is a live, user-reachable fallback on the homepage board, not only a comment); the shared enrichment pipeline (`_shared/enrich/types.ts`, `pdl.ts`, `builtwith.ts`, `tranco.ts`) refers throughout to an undocumented persona called "Mindy" and a "Diagnosis Room," neither named in `00_NORTH_STAR.md` or `01_CANON.md`; and `_shared/enrich/orchestrate.ts` still carries the retired offer-ladder vocabulary as internal-only data-model keys (`recommendedMode: "teardown"`, `handoverBand`), with a comment pointing at `src/lib/offers.ts`, which does not exist. None of this reaches public copy today, but `00_NORTH_STAR.md`'s naming law makes no exception for internal-only code, and "Mindy" in particular reads as an undisclosed chatbot persona, which the canon separately bans. Needs a code cleanup pass; not fixed by this docs-only pass.
12. **`/new-age-leadership` runs its own, unsanctioned entrance mechanism.** `src/components/new-age/AgathaStory.tsx`, rendered on that live route, uses a second `IntersectionObserver` (for an analytics beacon) and framer-motion `whileInView` entrance animations, outside `src/hooks/useReveal.ts` and outside every guarantee `03_DESIGN_CONTRACT.md`'s motion law makes for revealed content (readable if the reveal never fires, reduced-motion, no observer anywhere else). Found by this pass on 30 August 2026; needs a code fix to bring the page onto the sanctioned primitive, or a deliberate, documented exception if Krish wants to keep it as-is.
13. **The proof set has an unresolved quote-attribution question.** `04_PROOF.md` previously listed four excerpts under Steph Darmanin that do not match `src/data/testimonials.ts`: one is coded under Dipti Divekar, one is a close paraphrase of James Gately's coded quote, and two have no match under any name. `04_PROOF.md` now reproduces what the code actually renders and flags the mismatch, but which attribution is factually correct needs Krish to check against the original consent records before any of these four excerpts is reused publicly under a specific name.
14. **Two unreferenced logo assets were added directly to the repository.** `src/assets/builtwithai-logo-wordmark.png` and `src/assets/moneyofai-logo-wordmark.png` landed in commit `6e96519` ("Add files via upload," 29 August 2026) and are not imported or rendered anywhere in `src/`. `02_PUBLICATION.md` does not mention channel wordmarks. Needs a decision: wire them in (where, and to what spec) or remove them.
