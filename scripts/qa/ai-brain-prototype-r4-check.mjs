...
const base="http://127.0.0.1:4192/prototypes/ai-brain-vnext-r4/";
const output="C:/Users/krish/.scratch/mindmake-ai-brain-vnext-r4";
...
for(const [name,width,height] of viewports){
  ...
  const opening=await page.evaluate(()=>{const hero=document.querySelector(".hero");return {text:document.body.innerText.length,hero:hero.getBoundingClientRect().height,hOverflow:document.documentElement.scrollWidth-innerWidth,heroOverflow:hero.scrollHeight-hero.clientHeight,nodes:document.querySelectorAll(".living-node").length,edges:document.querySelectorAll(".living-edge").length,animation:getComputedStyle(document.querySelector(".living-viewport")).animationName}});
  ...
  fail(brainTransformBefore===brainTransformAfter,`${name}: Living Brain motion is not observable`);
  ...
  for(let stage=0;stage<4;stage++){
    ...
    if(stage===1){
      ...
      const graph=await page.evaluate(()=>{const sheet=document.querySelector(".decision-sheet").getBoundingClientRect();const map=document.querySelector(".living-brain").getBoundingClientRect();return {map,sheet,title:document.querySelector("#brainMapTitle").textContent.toLowerCase(),evidence:document.querySelector("#brainMapEvidence").textContent.toLowerCase(),version:document.querySelector("#brainMapVersion").textContent.toLowerCase(),selected:document.querySelectorAll(".living-node.is-active").length,activeEdges:document.querySelectorAll(".living-edge.is-active").length}});
...
await browser.close();
console.log(JSON.stringify({artifact:"ai-brain-vnext-r4",output,failures},null,2));
