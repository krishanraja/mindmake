...
The active source lock is `quality/route-lock/approved-production.lock.json`,
generated by `npm run qa:approved-routes:update` and checked by
scripts/qa/approved-route-lock-check.mjs. It continues r48; the numbered
manifests are frozen evidence, not current instructions. Preserve the deterministic adapter and accepted source.
...
- The menu and the About page, from Krish's request on 25 September 2026,
  carried by the generated lock `quality/route-lock/approved-production.lock.json`
  (numbered records stopped at r48) and that day's history log entry.
