...
describe("homepage pin lifecycle", () => {
  ...
  function fixture() {
    ...
    root.innerHTML = '<section data-component="history"></section>';
    ...
    const callbacks = { selectStory: vi.fn() };
  ...
  for (const fallback of ["reduced-motion", "short-height"] as const) {
    for (const chapter of ["history"] as const) {
      it(`resynchronises ${chapter} after ${fallback}, manual selection and re-enable`, () => {
        ...
        f.callbacks.selectStory(3);
        f.root.dispatchEvent(new CustomEvent("homepage:choice", { detail: { section: chapter, index: 3 } }));
        expect(f.callbacks.selectStory).toHaveBeenLastCalledWith(3);
        ...
        expect(section.dataset.scrollStage).toBe("0");
        expect(f.callbacks.selectStory).toHaveBeenLastCalledWith(0);
        f.destroy();
      });
    }
  }
