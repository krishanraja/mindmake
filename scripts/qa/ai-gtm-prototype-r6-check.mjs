...
const base = `${process.env.MINDMAKE_QA_ORIGIN ?? "http://127.0.0.1:4192"}/prototypes/ai-gtm-vnext-r6/`;
const output = "C:/Users/krish/.scratch/mindmake-ai-gtm-vnext-r6";
const viewports = [
...
for(const [name,width,height] of viewports){
  ...
  for(const signal of ["pricing","commerce","product","content","service"]){
    ...
    const choices=await page.evaluate(()=>{
      const group=document.querySelector(".response-tabs").getBoundingClientRect();
      const buttons=[...document.querySelectorAll(".response-tabs button")];
      return {group,buttons:buttons.map(button=>{const rect=button.getBoundingClientRect();const label=button.querySelector("b");const lineHeight=parseFloat(getComputedStyle(label).lineHeight);return {text:label.textContent.trim(),rect,overflow:button.scrollWidth>button.clientWidth+1||button.scrollHeight>button.clientHeight+1,lines:Math.round(label.getBoundingClientRect().height/lineHeight)}})};
    });
    const forbidden=/^(keep the seat|meter the work|price the result|keep website-first|syndicate the catalogue|build for agent buying|keep the spec gate|prototype before commitment|prototype against evidence|protect the page|license the evidence|build the intelligence product|lead with features|name the completed job|prove the operating model)$/i;
    fail(choices.buttons.some(choice=>forbidden.test(choice.text)||choice.text.split(/\s+/).length<4),`${name}: ${signal} retains contextless response language`);
    fail(choices.buttons.some(choice=>choice.overflow||choice.lines>2),`${name}: ${signal} response label clips or spiders over more than two lines`);
    if(width<=960){
      fail(choices.buttons.some(choice=>choice.rect.width<choices.group.width*.94),`${name}: ${signal} mobile responses are squeezed into desktop columns`);
    }
  }
...
await browser.close();
console.log(JSON.stringify({artifact:"ai-gtm-vnext-r6",output,failures},null,2));
