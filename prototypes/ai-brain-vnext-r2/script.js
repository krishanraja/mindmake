...
const states = ["Portrait", "Map", "Evidence", "Change"];
...
const brainItems = {
  builder: ["Aim", "4 source references · version 2"],
  possibility: ["Pattern", "2 source references · version 1"],
  release: ["Standard", "2 source references · version 2"],
  teachable: ["Aim", "3 source references · version 1"],
  convergence: ["Standard", "1 source reference · version 1"],
};
...
brainNodes.forEach((node) => node.addEventListener("click", () => {
  const [standing, evidence] = brainItems[node.dataset.brainItem];
  brainNodes.forEach((item) => {
    const selected = item === node;
    item.classList.toggle("is-active", selected);
    item.setAttribute("aria-pressed", String(selected));
  });
  brainMapReadout.querySelector("strong").textContent = standing;
  brainMapReadout.querySelector("span").textContent = evidence;
}));
