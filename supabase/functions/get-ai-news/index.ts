...
import { boardDaysRequested, buildBoard, type SharedPoolRow } from "./board.ts";
...
/**
 * Reads the retained window of cached days for the board view.
 *
 * The board publishes what moved across the whole window, so it needs each
 * day's full card list rather than the flattened headline shape. This reads the
 * same cache the pool reader above uses, and nothing else: the board is a claim
 * about corroborated data, so it never falls through to a generated ladder.
 */
const fetchBoardRows = async (days: number): Promise<SharedPoolRow[]> => {
  const url = Deno.env.get("SUPABASE_URL");
  const key = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if (!url || !key) return [];
  try {
    const res = await fetch(
      `${url}/rest/v1/live_headlines_cache?select=created_at,payload&order=created_at.desc&limit=${days}`,
      { headers: { apikey: key, Authorization: `Bearer ${key}` } },
    );
    if (!res.ok) return [];
    const rows = await res.json();
    return Array.isArray(rows) ? rows as SharedPoolRow[] : [];
  } catch (e) {
    console.warn("Board rows fetch failed:", e);
    return [];
  }
};
...
const parseLLMResponse = (content: string): NewsHeadline[] => {
  ...
  const valid = (Array.isArray(parsed) ? parsed : [])
    .filter((h): h is { title: string; source: string } => {
      const item = h as { title?: unknown; source?: unknown } | null;
      return typeof item?.title === 'string' && item.title.length > 15 && typeof item.source === 'string';
    })
...
const generateMarketPulseHeadlines = async (): Promise<NewsHeadline[]> => {
  ...
  try {
    ...
    /* The upstream payload is loosely typed, so the fields this reads are
       named here and everything else is ignored. */
    interface RawModel {
      name?: string;
      model_creator?: { name?: string };
      evaluations?: { quality_index?: number; arena_elo?: number };
      pricing?: {
        input_per_million_tokens?: number; input?: number;
        output_per_million_tokens?: number; output?: number;
      };
      median_output_tokens_per_second?: number;
    }

    const models: SimplifiedModel[] = rawModels
      .map((m: RawModel): SimplifiedModel | null => {
...
serve(async (req) => {
  ...
  // ── BOARD VIEW ──
  // Additive: only a request that asks for it takes this path, so every
  // existing consumer keeps the headline response byte for byte.
  try {
    const body = req.method === 'POST' ? await req.clone().json().catch(() => null) : null;
    const boardDays = boardDaysRequested(body);
    if (boardDays !== null) {
      const board = buildBoard(await fetchBoardRows(boardDays), boardDays);
      return new Response(
        JSON.stringify(board ?? { days: [], timestamp: null, provider: 'ctrl-shared-pool', total: 0 }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 },
      );
    }
  } catch (e) {
    console.error('Board view failed:', e);
  }
