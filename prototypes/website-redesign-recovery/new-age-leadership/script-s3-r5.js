(() => {
  ...
  const jumps = [...root.querySelectorAll('[data-reach-jump]')];
  let phase = '';
  ...
  const setPhase = next => {
    ...
    jumps.forEach(jump => {
      const active = jump.dataset.reachJump === phase;
      if (active) jump.setAttribute('aria-current', 'step');
  ...
  jumps.forEach(jump => jump.addEventListener('click', () => {
    const distance = Math.max(1, root.offsetHeight - sticky.offsetHeight);
    const progress = jump.dataset.reachJump === 'organisation' ? .68 : .12;
    scrollTo({ top: root.offsetTop + distance * progress, behavior: 'smooth' });
  }));
  if (location.hash === '#reach-title') {
    requestAnimationFrame(() => scrollTo(0, root.offsetTop));
  }
  update();
})();
