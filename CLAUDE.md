...
- `src/components/mindmake/`: the public design and conversion system, including the film plate, the marquee, the ask bar, the questions section (`ObjectionChips`, which is a stack and not a drum since 1 September 2026), the live board, the shared details capture (`DetailsJourney`, used by both `BrainJourney` and `GtmJourney`), and the lead journey (`LeadBrief`, `MindmakeProposal`, `proposalContent`, `privateBriefHtml`, `leadDelivery`, `companyRead`). Also `OneAtATime`, the stack where one row is open: a real `<details>` group, so the accordion runs with no script and every answer is in the markup, which is what the drum it replaced was not. Also `StoryIndex`, the card index: eight client stories as a deck you flick, driven by `useDragDrum` through its `write` option, which lands the offset on a custom property so CSS can put every card in one grid cell. Also `ClimbLadder`, `ProcessTrack` and `LeverPanel`, the three scrubbed section builds. Also `Instrument` (the six-mark set), `ProofDrum` with `useDragDrum` (the 33 voices), `StoryFigure` (the five proof diagrams), `FounderNote`, `SubscribeBand`, `MindmakeBrand`, `MobileChapter` and `MobileActionBar`.
...
`npm run qa:alive` is red on purpose, and its failure list is the worklist: 3 still viewports at 390 and 1 at 1440. Do not lower a floor to make it green; the named viewports are the thing to fix. Re-measured 2 September 2026 by driving each page: `/ai-brain @3376px` and `/ai-gtm @2532px` at 390 and `/ai-brain @3600px` at 1440 are all `.mm-try`, and `/ @844px` is the fourth. Splitting the try-it panels was predicted to wake them and did not, and shortening the copy around them does not either. A form screen is finished the moment it is drawn, and `Arrive` cannot help: the gate reads a page at rest, and an arrival has by then arrived. What those screens want is a set-piece, which is a design decision rather than a fix.
...
The current baselines, recorded in `project-documentation/06_CURRENT_STATE.md`: 395 tests, 0 lint errors and 2 warnings, 0 type errors. Do not add new lint problems. Point the typecheck at `tsconfig.app.json`: the root config carries `"files": []` and checks nothing.
