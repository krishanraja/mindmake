...
const base = flag("base", "http://127.0.0.1:4192/prototypes/ai-gtm-vnext-r2/");
const output = "C:/Users/krish/.scratch/mindmake-ai-gtm-vnext-r2";
const viewports = [
  { name: "desktop-1440x900", width: 1440, height: 900 },
  { name: "scaled-wide-1440x700", width: 1440, height: 700 },
  { name: "scaled-shallow-1536x696", width: 1536, height: 696 },
  { name: "reported-pixels-1882x870", width: 1882, height: 870 },
...
async function waitForScrollSettled(page) {
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    stableFrames = previous !== null && Math.abs(current - previous) < 1 ? stableFrames + 1 : 0;
...
for (const viewport of viewports) {
  ...
  const opening = await page.evaluate(() => {
    ...
    return {
      ...
      heroTitleTop: title.top,
      heroSafeTop: Math.max(header.bottom + 4, frame.top),
  ...
  fail(opening.heroTitleTop < opening.heroSafeTop - 1, `${viewport.name}: hero title begins above its safe content area by ${(opening.heroSafeTop - opening.heroTitleTop).toFixed(1)}px`);
...
await browser.close();
console.log(JSON.stringify({ artifact: "ai-gtm-vnext-r2", viewports: observations, screenshotDirectory: output, failures }, null, 2));
