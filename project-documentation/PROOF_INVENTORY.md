<!-- Generated 2026-08-05. Updated 2026-08-16. A working document for the proof rebuild, not a canonical reference. -->

> **Status, 2026-08-16.** Verified against the current codebase. **`project-documentation/BRANDS_AND_TESTIMONIALS.md` is now the single current source for public proof** (approved brands, the eight verified client outcome stories, and career references), matching `src/data/rebuildProof.ts` exactly. This file is kept as historical working detail only: the richer R-01 to R-09 narratives below (verified numbers, unpublished engagements) have no equivalent in the newer file and are still useful for future proof decisions, but its structural references are out of date. `src/data/caseStudies.ts` and `src/components/proof/CaseStudyCard.tsx`, both cited below as "what renders on `/case-studies`", were deleted in commit `cda3c70` (2026-08-12); `/case-studies` now renders from `src/data/rebuildProof.ts`. The `/handover` and `/teardown` routes cited below as publish destinations for R-01 no longer exist as pages; both engagements were retired 2026-08-12 (`DECISIONS_LOG.md`) and now redirect to `/sprint`. R-01's $254K figure is not currently published anywhere on the live site. Treat every route, offer-tag and "currently renders" claim below as a snapshot of the pre-2026-08-12 estate, not current fact.
# Proof Inventory
