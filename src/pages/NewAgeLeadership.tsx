...
function useExperience(rootRef: React.RefObject<HTMLDivElement>) {
  useEffect(() => {
    ...
    /* The six AI Brain benefits used to advance themselves every 6.2 seconds.
       On a page whose whole argument is carried by scrolling, that was the one
       element that moved on its own, and it changed the sentence under anybody
       reading at their own pace. It is now a pinned sequence the reader drives,
       like the three before it, and the arrows move the page to a state rather
    ...
    const setBenefit = (raw: number, announce = false) => {
      ...
      /* The rule under the arrows reads this. Nothing set it before, so the
         one piece of progress feedback the controls had never moved. */
    ...
    /* Every sequence reads its state from where the page is, forwards and
       backwards. The history lens is no longer gated to wide screens: below
       900px it pins too, so there is a scroll position to read. The work
       scenes stay gated, because below 900px they are laid out one after
       another and the reader is already moving through them. */
    const update = () => { if (lens && performance.now() >= lensLockUntil) setLens(Math.round(Math.max(0, Math.min(1, -lens.getBoundingClientRect().top / Math.max(1, lens.offsetHeight - innerHeight))) * 3)); const work = root.querySelector<HTMLElement>(".work-scroll"); if (work && innerWidth > 900) setWork(Math.round(Math.max(0, Math.min(1, -work.getBoundingClientRect().top / Math.max(1, work.offsetHeight - innerHeight))) * 2)); if (reach && reachSticky) { const p = Math.max(0, Math.min(1, -reach.getBoundingClientRect().top / Math.max(1, reach.offsetHeight - reachSticky.offsetHeight))); reach.style.setProperty("--reach-progress", String(p)); if (performance.now() >= reachLockUntil) setReach(p < 0.48 ? "boundary" : "organisation"); } if (benefitTrack && benefitPanel && performance.now() >= benefitLockUntil) setBenefit(Math.round(Math.max(0, Math.min(1, -benefitTrack.getBoundingClientRect().top / Math.max(1, benefitTrack.offsetHeight - benefitPanel.offsetHeight))) * (benefitNodes.length - 1))); };
    ...
    root.querySelector("[data-benefit-prev]")?.addEventListener("click", () => chooseBenefit(benefit - 1), { signal: abort.signal }); root.querySelector("[data-benefit-next]")?.addEventListener("click", () => chooseBenefit(benefit + 1), { signal: abort.signal });
    setLens(0); setWork(0); setBenefit(0); update();
