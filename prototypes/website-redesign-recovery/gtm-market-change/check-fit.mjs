...
async function openPage(label, viewport, query = "") {
  ...
  monitor(page, label);
  await page.goto(`${conceptUrl}index-fit.html${query}`, { waitUntil: "networkidle" });
...
await reducedPage.goto(`${conceptUrl}index-fit.html`, { waitUntil: "networkidle" });
...
await noJsPage.goto(`${conceptUrl}index-fit.html`, { waitUntil: "networkidle" });
...
await review.goto(`${conceptUrl}review-fit.html`, { waitUntil: "networkidle" });
await review.evaluate(() => document.fonts.ready);
await review.screenshot({ path: `${conceptDir}paired-fit.png` });
await review.close();

const desktopShot = await browser.newPage({ viewport: { width: 1440, height: 1100 } });
monitor(desktopShot, "desktop screenshot");
await desktopShot.goto(`${conceptUrl}index-fit.html?render=final`, { waitUntil: "networkidle" });
...
await desktopShot.screenshot({ path: `${conceptDir}desktop-fit-1440.png`, fullPage: true });
...
await mobileShot.goto(`${conceptUrl}index-fit.html?render=final`, { waitUntil: "networkidle" });
...
await mobileShot.screenshot({ path: `${conceptDir}mobile-fit-390.png`, fullPage: true });
