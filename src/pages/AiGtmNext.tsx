...
import "@/styles/mindmake-gtm-next.css";

/* Review candidate for the plain-English /ai-gtm (mock r2). Not linked or
   indexed: it reuses the approved GTM surface's markup vocabulary, films and
   scroll build so the owner can judge copy and flow inside the real system. */
...
const SOURCES = {
  product: { name: "Uber", date: "Apr 2026", href: "https://www.uber.com/us/en/blog/ai-prototyping/" },
  price: { name: "HubSpot", date: "Apr 2026", href: "https://www.hubspot.com/company-news/hubspots-customer-agent-and-prospecting-agent-now-you-pay-when-the-task-is-complete" },
  positioning: { name: "Shopify", date: "Mar 2026", href: "https://www.shopify.com/news/agentic-commerce-momentum" },
  people: { name: "Zendesk", date: "May 2026", href: "https://www.zendesk.com/newsroom/press-releases/relate-2026/" },
} as const;
...
/* Scroll progress of one element through the viewport, 0 before it arrives and
   1 once it has fully settled. Reverses when the reader scrolls back up. */
...
export default function AiGtmNext() {
  ...
  return (
    <MindmakeShell onStart={() => openBrief("gtm")} mainClassName="mm-locked-route-main" siteClassName="mm-route-gtm" showMobileActionBar={false} compactFooter>
      <SEO title="Build your AI go-to-market" description="Make your pricing, positioning and team AI-native, then test it with real buyers." canonical="/ai-gtm" noindex />
      <div ref={rootRef} className="mm-locked-gtm mm-gtm-next no-js" data-mode={mode}>
        <div className="page-shell">
          <section className="intro" aria-labelledby="page-title">
            <figure className="motion-scene motion-scene--threshold" aria-hidden="true" data-motion-scene="threshold">
              <video className="motion-film" data-motion-video="threshold" data-src={quietWorkshopFilm} poster={quietWorkshopPoster} preload="none" muted loop playsInline />
          ...
          <section className="signal-deck next-deck" aria-labelledby="business-heading">
            <figure className="motion-scene motion-scene--signals" aria-hidden="true" data-motion-scene="signals">
              <video className="motion-film" data-motion-video="signals" data-src={signalsArriveFilm} poster={signalsArrivePoster} preload="none" muted loop playsInline />
              <span className="motion-scrim" />
            </figure>
          ...
          <section className="linkage next-levers" aria-labelledby="levers-heading">
            ...
            >
              ...
              <div className="mobile-stage" aria-live="polite">
                <div className="mobile-link" aria-hidden="true">
                  <span className="link-pin" /><i /><span className="link-pin" />
                </div>
                <article className="paper-leaf mobile-leaf">
                  <span className="leaf-index">{String.fromCharCode(65 + stage)} / 04</span>
                  <h3>{LEVER_NAMES[stageKey]}</h3>
                  <p>{copy.levers[stageKey].now}</p>
                  <div className="leaf-was">
                    <span><s>{copy.levers[stageKey].was}</s></span>
                    <a href={SOURCES[stageKey].href} target="_blank" rel="noreferrer">{SOURCES[stageKey].name} · {SOURCES[stageKey].date} <span aria-hidden="true">&#8599;</span></a>
                  </div>
                </article>
                <div className="stage-controls">
                  <button type="button" onClick={() => setStage((value) => Math.max(0, value - 1))} disabled={stage === 0} aria-label="Show previous change">
                    <span aria-hidden="true">&larr;</span> Back
                  </button>
                  <p>{stage + 1} of 4</p>
                  <button type="button" onClick={() => setStage((value) => (value + 1) % LEVERS.length)}>
                    {stage === LEVERS.length - 1 ? "Back to Product" : `Next: ${LEVER_NAMES[LEVERS[stage + 1]]}`} <span aria-hidden="true">&rarr;</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <section className="linkage next-team" aria-labelledby="team-heading">
            ...
            <div ref={teamRef} className="team-board" data-view={view} style={{ "--team-progress": teamProgress.toFixed(4) } as React.CSSProperties}>
              <div className="team-lead">
                <RoleTag role={copy.lead[view]} active={picked === "lead"} onPick={() => setPicked("lead")} />
              </div>
              <div className="team-roles">
                {roles.map((item) => (
                  <RoleTag key={`${view}-${item.id}`} role={item} active={picked === item.id} onPick={() => setPicked(item.id)} />
                ))}
              </div>
            </div>
...
function RoleTag({ role, active, onPick }: { role: Role; active: boolean; onPick: () => void }) {
  return (
    <button type="button" className={`role-tag kind-${role.kind}${role.isNew ? " is-new" : ""}`} aria-pressed={active} onClick={onPick}>
      <span className="role-kind"><i aria-hidden="true">{KIND_MARK[role.kind]}</i>{KIND_NAME[role.kind]}</span>
      <b>{role.title}</b>
