...
Historical record, not current guidance. These are the verbatim originals superseded by the 24 September 2026 documentation consolidation: twelve documents and two fragments. On 26 September 2026 they moved here from [LOG.md](LOG.md), byte for byte, so the log keeps to dated entries as the docs steward requires. Each copy keeps its anchor, source path, raw SHA-256 and byte count, and LOG.md keeps a pointer at every anchor. `scripts/qa/website-restart-contract-check.mjs` verifies the twelve stored document bodies against their recorded hashes.

## Twelve documents from the 24 September 2026 consolidation
...
## Two fragments: remaining current-document originals
