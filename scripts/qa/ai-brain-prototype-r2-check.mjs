...
const base = flag("base", "http://127.0.0.1:4192/prototypes/ai-brain-vnext-r2/");
const output = "C:/Users/krish/.scratch/mindmake-ai-brain-vnext-r2";
const viewports = [
...
for (const viewport of viewports) {
  ...
  for (let stage = 0; stage < 4; stage += 1) {
    ...
    const stageMetrics = await page.evaluate((expected) => {
      ...
      return {
        ...
        controlHeights: [...document.querySelectorAll(`[data-panel="${expected}"] button`)].map((button) => button.getBoundingClientRect().height),
        horizontalOverflow: document.documentElement.scrollWidth - document.documentElement.clientWidth,
      };
    }, stage);
    ...
    fail(stageMetrics.controlHeights.some((height) => height < 44), `${viewport.name}: stage ${stage} has a control below 44px`);
    ...
    if (stage === 1) {
      await page.locator('[data-brain-item="release"]').click();
      const mapState = await page.evaluate(() => ({
        active: document.querySelector('.brain-node[aria-pressed="true"]')?.dataset.brainItem,
        readout: document.querySelector("#brainMapReadout")?.innerText,
      }));
      fail(mapState.active !== "release" || !mapState.readout?.toLowerCase().includes("2 source"), `${viewport.name}: Brain map inspection does not expose evidence state`);
    }
  }
  ...
  const youState = await page.evaluate(() => {
    ...
    return {
      ...
      controlHeights: [...screen.querySelectorAll("button, input, select")].map((control) => control.getBoundingClientRect().height),
    };
  });
  ...
  fail(youState.controlHeights.some((height) => height < 44), `${viewport.name}: You screen has a control below 44px`);
  ...
  await page.locator('[data-drawer-screen="problem"]:not([hidden])').waitFor();
  const problemState = await page.evaluate(() => ({
    domain: document.querySelector("#problemDomain")?.textContent,
    person: document.querySelector("#problemPerson")?.textContent,
    current: document.querySelector("[data-drawer-step].is-current")?.dataset.drawerStep,
    focused: document.activeElement?.id,
    overflow: document.querySelector('[data-drawer-screen="problem"]').scrollHeight - document.querySelector('[data-drawer-screen="problem"]').clientHeight,
    controlHeights: [...document.querySelector('[data-drawer-screen="problem"]').querySelectorAll("button, textarea")].map((control) => control.getBoundingClientRect().height),
  }));
  fail(problemState.domain !== "example.com" || problemState.person !== "Test · Founder", `${viewport.name}: Problem screen loses company or person context`);
  fail(problemState.current !== "problem" || problemState.focused !== "decisionProblem", `${viewport.name}: Problem screen progress or focus is wrong`);
  fail(problemState.overflow > 1, `${viewport.name}: Problem screen internally scrolls by ${problemState.overflow}px`);
  fail(problemState.controlHeights.some((height) => height < 44), `${viewport.name}: Problem screen has a control below 44px`);
  await page.locator("#decisionProblem").fill("Choose the next market to enter");
  await page.locator('[data-drawer-screen="problem"] .drawer-action').click();
  ...
  const completionState = await page.evaluate(() => ({
    ...
    context: document.querySelector("#completeContext")?.textContent,
    current: document.querySelector("[data-drawer-step].is-current")?.dataset.drawerStep,
  ...
  fail(completionState.heading !== "Your starting point is intact.", `${viewport.name}: completion does not state truthful continuity`);
  fail(completionState.context !== "Choose the next market to enter" || completionState.current !== "complete", `${viewport.name}: completion loses the captured decision`);
  fail(completionState.nextAction !== "Finish preview →", `${viewport.name}: completion action is unclear`);
...
fail(!["builder of judgement", "Your mind is taking shape", "Human release judgement", "Consequential work waits for human review"].every((text) => reducedMetrics.sequenceText.includes(text)), "reduced motion: Brain story is incomplete");
...
await browser.close();
console.log(JSON.stringify({ artifact: "ai-brain-vnext-r2", viewports: observations, screenshotDirectory: output, failures }, null, 2));
