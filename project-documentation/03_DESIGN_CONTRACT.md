...
The AI Brain signature (BRAIN-NARRATIVE-S5, 26 September 2026, sharpening S4 of r48; receipt `quality/ai-brain/approved-narrative-s5.json`) speaks to the reader. After the evidence-connects hero come six chapters: You (five questions only the leader can answer, each labelled with what it draws on), Inside (the twenty-idea constellation; the panel's label names the picked idea's standing, Agreed by you or Your Brain's reading, and the picked idea's own connections light), Sharper (one decision: the standard, the question you tend to skip, your call), Private (the living record, which reaches four kinds of work named on its foot: board paper, client reply, next post, team brief), Business (one call reaching product, price, positioning and people) and Built (the month: thirty days on the Brain's rail light with four steps, Week 1, Weeks 2 to 4, Day 30 and After, then the rail carries on dashed). The page names jobs and outcomes, never AI companies, and carries no eyebrows, captions or stray lines. The stepped instruments light by colour and opacity only, forward and back with scroll, and read whole without script, with reduced motion or on short screens. Chapters after the opening take the height left under the fixed header and centre above the action bar and privacy notice from the heights those publish. Grounds reuse the Brain's own: sand with the clay band (You and Built), paper, teal and amber. Portrait phones use a readable natural-height document.
