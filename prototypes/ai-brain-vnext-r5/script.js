...
const states = ["Decision", "Brain", "Evidence", "Correction"];
...
const livingGraph = document.querySelector(".living-graph");
const livingGraphSvg = document.querySelector("#livingBrainGraph");
...
function brainLayout(items) {
  const width = 880;
  const ratio = livingGraph.clientWidth / Math.max(1, livingGraph.clientHeight);
  const height = Math.max(520, Math.min(1800, width / Math.max(.45, ratio)));
  const minX = Math.min(...items.map((item) => item.x));
  const maxX = Math.max(...items.map((item) => item.x));
  const minY = Math.min(...items.map((item) => item.y));
  const maxY = Math.max(...items.map((item) => item.y));
  return { left: 60, top: 40, width, height, minX, maxX, minY, maxY, padX: 38, padY: 34 };
}

function brainPoint(item, layout) {
  const x = layout.left + layout.padX + ((item.x - layout.minX) / (layout.maxX - layout.minX)) * (layout.width - layout.padX * 2);
  const y = layout.top + layout.padY + ((item.y - layout.minY) / (layout.maxY - layout.minY)) * (layout.height - layout.padY * 2);
  return { x, y };
}
...
function renderLivingBrain(data) {
  brainFixture = data;
  const layout = brainLayout(data.items);
  livingGraphSvg.setAttribute("viewBox", `${layout.left} ${layout.top} ${layout.width} ${layout.height}`);
  const clusterRows = [.12, .1, .46, .88, .88, .95];
  document.querySelectorAll(".living-clusters text").forEach((label, index) => {
    label.setAttribute("y", String(layout.top + layout.height * clusterRows[index]));
  });
  const itemById = new Map(data.items.map((item) => [item.id, item]));
  livingEdges.innerHTML = data.relationships.map((relationship) => {
    const from = brainPoint(itemById.get(relationship.from), layout);
    const to = brainPoint(itemById.get(relationship.to), layout);
  ...
  livingNodes.innerHTML = data.items.map((item, index) => {
    const point = brainPoint(item, layout);
...
function updateScroll() {
  if (matchMedia("(max-width: 60rem) and (max-height: 43.75rem) and (orientation: portrait)").matches) return;
...
function scrollToStage(stage) {
  ...
  const narrowViewport = matchMedia("(max-width: 60rem)").matches;
  window.scrollTo({ top, behavior: narrowViewport || matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" });
}
